...
│   └── .../stats/       成績一覧（リーグ全選手の通算成績。野手／投手・規定到達で
│                        切替、列で並べ替え）
├── /teams/              チーム一覧
...
（最新シーズン）です。**チームの一覧とは切り替えません**（順位表が同じチームを
同じ選手名簿へのリンクで並べているため、二重になります）。ただしそのシーズンに
1試合も行われていないリーグでは順位表が作れないので、そのときだけチームの一覧に
差し替えます（登録したばかりのチームが概況のどこにも出ない状態を避けるため）。
所在地・在籍人数まで見るときは、カードヘッダーの「チーム一覧」から移ります。

順位表の下には**直近の試合**（5件）を置きます。順位表が「どこが強いか」を示すのに対して、
こちらは「いま何が起きているか」を示します。**カードに `h-100` は付けません** —
左のランキング2段（982px）に合わせて伸ばすと、6行の順位表しかない右側のカードの中に
500px超の空洞が残ります。高さの差はこの2枚目のカードで埋めます。直近の試合は順位表と
同じ `league_games`（明細を持たない一覧）から作るので、**追加のクエリは増えません**
（実測 435ms / 13クエリ。組み込み前と同じ）。

上部の集計は**リーグ・チーム・選手**の3つです。野手と投手を分けて数えても概況としては
読めないので、`Dashboard.player_count` で合算します（内訳が要る場面はチームやリーグの
成績一覧にあります）。
...
成績一覧では**規定に到達した選手だけに絞れます**。率の指標（打率・防御率・OPS）は
規定未満の選手が混ざると読めません（1打数1安打が打率10割で並ぶため）。

- 規定打席 ＝ チームの試合数 × 3.1、規定投球回 ＝ チームの試合数 × 1.0（NPB の規則）
- 条件は**タイトル一覧と同じドメインサービス**（`qualified_batters` /
  `qualified_pitchers`）が持ちます。画面側に別の条件を書くと、タイトルの対象と
  一覧の対象がずれます
- 切り替えの前に規模が分かるよう、**どちらの状態でも両方の人数**を出します
  （「全員 104 / 規定打席到達 54」）
- 野手／投手の切替と列の並べ替えとは互いに独立で、片方を押しても他方の指定は
  外れません。不正な指定はエラーにせず「全員」に落とします
