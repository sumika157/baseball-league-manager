...
├── /league/<id>/        リーグ画面（所属チーム・順位表・直近の試合）
├── /teams/              チーム一覧
...
**閲覧は誰でも、書き込みはログインした人だけ**という一本の線で分けています。

| | ログイン不要 | ログイン必須 |
| --- | --- | --- |
| 試合 | 一覧・詳細 | 登録・記録の編集 |
| 選手 | 一覧・個人ページ・検索 | 登録・編集・退団 |
| その他 | ダッシュボード・順位表・リーグ・チーム一覧 | — |

書き込みの導線（登録フォームや「編集」ボタン）は、**ログインしていない人には
表示しません**。押しても弾かれるだけの導線を見せないためです。

選手一覧のように読み書きが同じ URL に同居する画面は、GET は誰でも通し、
POST だけログインを求めます（画面ごと `login_required` にすると閲覧まで塞がるため）。
